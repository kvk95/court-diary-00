...
class CalendarController(BaseController):
    ...
    async def calendar_get_month(
        self,
        year: int = Query(..., ge=2000, le=2099, description="Year e.g. 2025"),
        month: int = Query(..., ge=1, le=12, description="Month 1-12"),
        service: CalendarService = Depends(get_calendar_service),
    ) -> BaseOutDto[CalendarMonthOut]:
        return self.success(result=await service.calendar_get_month(year=year, month=month))
    ...
    @BaseController.get(
        ...
        response_model=BaseOutDto[List[CalendarUpcomingOut]],
    )
    async def calendar_get_upcoming(
        ...
        service: CalendarService = Depends(get_calendar_service),
    ) -> BaseOutDto[List[CalendarUpcomingOut]]:
        return self.success(
    ...
    async def calendar_get_range(
        ...
        return self.success(
            result=await service.calendar_get_range(date_from=date_from, date_to=date_to)
        )
