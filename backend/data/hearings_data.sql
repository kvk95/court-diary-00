...
SET @user_caadmin   = (SELECT user_id FROM users WHERE email = 'admin@vkchamber.in');
...
(@chamber_ca, @case1, DATE_SUB(@today, INTERVAL 4 DAY), 'HSUP', 'HPAR', 'Final arguments pending', @today, @user_priya),
(@chamber_ca, @case2, DATE_SUB(@today, INTERVAL 2 DAY), 'HSUP', 'HPEV', 'Witness examination ongoing', @today, @user_priya),
(@chamber_ca, @case3, DATE_SUB(@today, INTERVAL 6 DAY), 'HSUP', 'HPAD', 'Anticipatory bail hearing', @today, @user_priya),

-- =========================
-- TODAY'S HEARINGS (Current Date)
-- =========================
-- Logic: Hearing date is @today, Next Hearing is NULL (To be decided in court)
(@chamber_ca, @case1, @today, 'HSUP', 'HPAR', 'Final arguments hearing', NULL, @user_priya),
(@chamber_ca, @case2, @today, 'HSUP', 'HPEV', 'Evidence stage hearing', NULL, @user_priya),
(@chamber_ca, @case3, @today, 'HSUP', 'HPAR', 'Arguments stage', NULL, @user_priya),

-- =========================
-- UPCOMING THIS WEEK (Near Future)
-- =========================
-- Logic: Hearing dates are +2 to +4 days from @today
(@chamber_ca, @case4, DATE_ADD(@today, INTERVAL 2 DAY), 'HSUP', 'HPAD', 'First hearing (admission)', NULL, @user_priya),
(@chamber_ca, @case5, DATE_ADD(@today, INTERVAL 3 DAY), 'HSUP', 'HPME', 'Mediation scheduled', NULL, @user_priya),
(@chamber_ca, @case6, DATE_ADD(@today, INTERVAL 4 DAY), 'HSUP', 'HPEV', 'Document evidence submission', NULL, @user_priya),

-- =========================
-- FUTURE HEARINGS (Next Month)
-- =========================
-- Logic: Hearing dates are ~3-4 weeks from @today
(@chamber_ca, @case7, DATE_ADD(@today, INTERVAL 22 DAY), 'HSUP', 'HPAR', 'Arguments to be presented', NULL, @user_priya),
(@chamber_ca, @case8, DATE_ADD(@today, INTERVAL 27 DAY), 'HSUP', 'HPPL', 'Pleadings stage', NULL, @user_priya),

-- =========================
-- ADJOURNED CASE (Past Hearing, Future Next Date)
-- =========================
-- Logic: Hearing was 3 days ago, Next hearing is 12 days from now
(@chamber_ca, @case9, DATE_SUB(@today, INTERVAL 3 DAY), 'HSAD', 'HPAD', 'Adjourned due to counsel absence', DATE_ADD(@today, INTERVAL 12 DAY), @user_priya),

-- =========================
-- COMPLETED HISTORY (Older Past Dates)
-- =========================
-- Logic: Hearings are 1-2 weeks ago, Next Hearing was scheduled for @today
(@chamber_ca, @case1, DATE_SUB(@today, INTERVAL 9 DAY), 'HSCP', 'HPAD', 'Preliminary hearing completed', @today, @user_priya),
(@chamber_ca, @case2, DATE_SUB(@today, INTERVAL 6 DAY), 'HSCP', 'HPPL', 'Pleadings completed', @today, @user_priya),
(@chamber_ca, @case3, DATE_SUB(@today, INTERVAL 14 DAY), 'HSCP', 'HPAD', 'Initial hearing completed', @today, @user_priya);